...
interface AiTabProps {
  ...
  setTranslateEndpoint: (url: string) => void;
  onTranslateEndpointCommit?: (url: string) => void;
  translateModel: string;
  setTranslateModel: (model: string) => void;
  onTranslateModelCommit?: (model: string) => void;
}
...
export const AiTab: React.FC<AiTabProps> = ({
  ...
  // State danh sách Provider từ models.json
  ...
  // Nạp providers và feature profiles khi mount
  useEffect(() => {
    loadProviders();
    loadProfiles();
  }, []);
  ...
  return (
    <div className="space-y-8 pb-8">
      {/* 1. PHÂN HỆ QUẢN LÝ NHÀ CUNG CẤP AI (PROVIDER MANAGER) */}
      <section className="space-y-3">
        <ProviderManager
          providers={providers}
          onProvidersChanged={(newList) => {
            setProviders(newList);
          }}
        />
      </section>

      {/* 2. PHÂN HỆ CẤU HÌNH TÍNH NĂNG (FEATURE BINDINGS) */}
      ...
      {/* 3. TỪ VỰNG CHUYÊN NGÀNH (CUSTOM VOCABULARY) */}
